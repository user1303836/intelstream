...
describe("strict protocol v1", () => {
  it("decodes every control envelope and a populated final", () => {
    const messages = [
      ...
      { version: 2, type: "ticket", reconnect_ticket: "refreshed-ticket", refresh_id: "refresh-identifier" },
      { version: 2, type: "waiting", open_seats: 1 }, { version: 2, type: "ready", players: publicPlayers },
      { version: 2, type: "paused", player_id: "two", grace_ms: 20000 }, { version: 2, type: "resumed", player_id: "two" },
      { version: 2, type: "error", code: "room_full" }, envelope(),
      { version: 2, type: "final", match_id: "m", winner_id: "one", method: "decision", round: 12, scorecards: [{ judge: "A", player_one: [10], player_two: [9] }, { judge: "B", player_one: [9], player_two: [10] }, { judge: "C", player_one: [10], player_two: [9] }], ratings: { one: { before: 1500, after: 1516 }, two: { before: 1500, after: 1484 } } },
    ];
  ...
  it("decodes spectators without granting fighter fields", () => {
    const spectator = { version: 2, type: "welcome", role: "spectator", player_id: "viewer", players: publicPlayers, server_tick: 40, reconnect_ticket: "spectator-ticket" };
    ...
    const fighterWithoutRole = { version: 2, type: "welcome", player_id: "one", seat: 1, rating: 1500, players: [publicPlayers[0]], server_tick: 0, next_sequence: 0 };
  ...
  it("decodes a fully populated embedded result", () => {
    ...
    expect(decodeServerFrame(JSON.stringify({ version: 2, type: "snapshot", payload: populated })).type).toBe("snapshot");
  });
  it.each([
    ["missing", { version: 2, type: "waiting" }], ["extra", { version: 2, type: "waiting", open_seats: 1, winner_id: "forged" }],
    ["version", { version: 3, type: "waiting", open_seats: 1 }], ["nonfinite", "{\"version\":1,\"type\":\"paused\",\"player_id\":\"one\",\"grace_ms\":NaN}"],
  ...
  it("rejects duplicate, oversized, and same-player snapshots", () => {
    expect(() => decodeServerFrame('{"version":2,"type":"waiting","type":"ready","open_seats":1}')).toThrow(/duplicate/u);
  ...
  it("encodes only authorized fields, clamps movement, and caps actions", () => {
    ...
    expect(encoded).toMatchObject({ version: 2, type: "input", sequence: 3, client_tick: 9, move: { x: 0, y: -1000 } }); expect(encoded).not.toHaveProperty("winner_id"); expect((encoded.actions as unknown[])).toHaveLength(4); expect((encoded.actions as Record<string, unknown>[])[0]).not.toHaveProperty("winner_id");
  });
  it("accepts authoritative rounds 13–15 with exactly three complete judge cards", () => {
    ...
    const final = { version: 2, type: "final", match_id: "m", winner_id: "one", method: "decision", round: 15, scorecards: cards, ratings: { one: { before: 1500, after: 1516 }, two: { before: 1500, after: 1484 } } };
    expect(decodeServerFrame(JSON.stringify(final))).toMatchObject({ type: "final", round: 15 });
    const atThirteen = { ...snapshot(), round_number: 13 };
    expect(decodeServerFrame(JSON.stringify({ version: 2, type: "snapshot", payload: atThirteen }))).toMatchObject({ type: "snapshot", payload: { round_number: 13 } });
  });
  it.each([
    ...
    expect(() => decodeServerFrame(JSON.stringify({ version: 2, type: "final", match_id: "m", winner_id, method, round: 1, scorecards: cards, ratings: { one: { before: 1, after: 1 }, two: { before: 1, after: 1 } } }))).toThrow(/coherent/u);
  });
  it("accepts exact fighter-domain boundaries and the private get-up sentinel", () => {
    ...
    expect(decodeServerFrame(JSON.stringify({ version: 2, type: "snapshot", payload: { ...base, fighters: [lower, upper] } }))).toMatchObject({ type: "snapshot", payload: { fighters: [{ get_up_required: 45 }, { get_up_required: 169 }] } });
    const redactedOpponent = { ...upper, get_up_required: 0 };
    expect(decodeServerFrame(JSON.stringify({ version: 2, type: "snapshot", payload: { ...base, fighters: [lower, redactedOpponent] } }))).toMatchObject({ payload: { fighters: [{ get_up_required: 45 }, { get_up_required: 0 }] } });
  });
  it.each([
    ...
    expect(() => decodeServerFrame(JSON.stringify({ version: 2, type: "snapshot", payload: { ...base, fighters: [changed, base.fighters[1]] } }))).toThrow(ProtocolError);
  });
  it("rejects stamina above the decoded maximum", () => {
    const base = snapshot();
    const exhausted = { ...base.fighters[0], maximum_stamina: 330, stamina: 331 };
    expect(() => decodeServerFrame(JSON.stringify({ version: 2, type: "snapshot", payload: { ...base, fighters: [exhausted, base.fighters[1]] } }))).toThrow(ProtocolError);
  });
  it("rejects out-of-authority event and fighter bounds", () => {
    const base = snapshot();
    const badEvent = { version: 2, type: "snapshot", payload: { ...base, events: [{ event_id: 1, tick: 1, kind: "x".repeat(33), actor_id: null, target_id: null, amount: 10_001, detail: "", blood: 101, direction: 2 }] } };
    expect(() => decodeServerFrame(JSON.stringify(badEvent))).toThrow();
    const badQueue = { version: 2, type: "snapshot", payload: { ...base, fighters: [{ ...base.fighters[0], queued_actions: 2 }, base.fighters[1]] } };
    expect(() => decodeServerFrame(JSON.stringify(badQueue))).toThrow();
  });
  it("strictly decodes same-origin API payload schemas", () => {
    expect(decodeBootstrap({ client_id: "123", state: "s", protocol: 2, simulation: { tick_rate: 30, ring_half_width: 6000, ring_half_height: 4000 } }).simulation.tick_rate).toBe(30);
