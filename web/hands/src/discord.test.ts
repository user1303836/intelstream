...
describe("Discord SDK OAuth", () => {
  ...
  it("requires a reload when SDK construction fails", async () => {
    history.replaceState({}, "", launchUrl); vi.stubGlobal("fetch", vi.fn(async () => response({ client_id: "123", state: "state", protocol: 2, simulation: { tick_rate: 30, ring_half_width: 6000, ring_half_height: 4000 } })));
  ...
  it("bounds a missing SDK ready response and closes the stalled realm", async () => {
    vi.useFakeTimers();
    try {
      history.replaceState({}, "", launchUrl); const order: string[] = []; const mockSdk = sdk("launch-1", order); (mockSdk.ready as ReturnType<typeof vi.fn>).mockImplementation(() => new Promise(() => {})); vi.stubGlobal("fetch", vi.fn(async () => response({ client_id: "123", state: "state", protocol: 2, simulation: { tick_rate: 30, ring_half_width: 6000, ring_half_height: 4000 } })));
  ...
  it("rejects SDK/launch instance disagreement before authorize", async () => {
    history.replaceState({}, "", launchUrl); const order: string[] = []; const mockSdk = sdk("other", order); vi.stubGlobal("fetch", vi.fn(async () => response({ client_id: "123", state: "state", protocol: 2, simulation: { tick_rate: 30, ring_half_width: 6000, ring_half_height: 4000 } })));
