...
describe("authoritative state reducer", () => {
  it("moves through wait, ready, combat, pause, resume and final without client outcomes", () => {
    let state = reduceState(initialState, { type: "message", message: { version: 1, type: "welcome", role: "fighter", player_id: "one", seat: 1, rating: 1500, players: [publicPlayers[0]], server_tick: 0, next_sequence: 7 } });
    expect(state).toMatchObject({ role: "fighter", playerId: "one", nextSequence: 7 });
    state = reduceState(state, { type: "message", message: { version: 1, type: "waiting", open_seats: 1 } }); expect(state.stage).toBe("waiting");
    state = reduceState(state, { type: "message", message: { version: 1, type: "ready", players: [...publicPlayers] } }); expect(Object.keys(state.players)).toHaveLength(2);
    state = reduceState(state, { type: "message", message: { version: 1, type: "snapshot", payload: snapshot(20) } }); expect(state.stage).toBe("fight");
    state = reduceState(state, { type: "message", message: { version: 1, type: "paused", player_id: "two", grace_ms: 20000 } }); expect(state.stage).toBe("paused");
  ...
  it("stores spectator role without granting fighter identity or sequence authority", () => {
    const state = reduceState(initialState, { type: "message", message: { version: 1, type: "welcome", role: "spectator", player_id: "viewer", players: [...publicPlayers], server_tick: 30 } });
