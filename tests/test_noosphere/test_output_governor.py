...
class TestOutputGovernor:
    def test_gain_defaults_to_one(self) -> None:
        gov = OutputGovernor()
        assert gov.get_gain(1) == 1.0

    def test_gain_stays_one_below_threshold(self) -> None:
        gov = OutputGovernor(min_msg_threshold=5)
        for _ in range(3):
            gov.record_message(1, is_bot=False)
        gov.record_message(1, is_bot=True)
        assert gov.get_gain(1) == 1.0
    ...
    def test_should_send_respects_cooldown(self) -> None:
        ...
        gov.record_response(1)
        assert gov.should_send(1) is False

    def test_should_send_after_cooldown(self) -> None:
        gov = OutputGovernor(cooldown_seconds=0.0)
        gov.record_response(1)
        assert gov.should_send(1) is True
    ...
    def test_gain_cached_within_interval(self) -> None:
        ...
        for _ in range(20):
            gov.record_message(1, is_bot=False)
        gain1 = gov.get_gain(1)
        for _ in range(20):
            gov.record_message(1, is_bot=True)
        gain2 = gov.get_gain(1)
        assert gain1 == gain2
    ...
    def test_independent_channels(self) -> None:
        ...
        for _ in range(10):
            gov.record_message(1, is_bot=True)
        for _ in range(10):
            gov.record_message(2, is_bot=False)
        assert gov.get_gain(1) < gov.get_gain(2)
