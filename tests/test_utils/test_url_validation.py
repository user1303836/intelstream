...
class TestIsSafeUrl:
    ...
    @patch("intelstream.utils.url_validation.socket.gethostbyname")
    def test_blocks_private_ip_resolution(self, mock_gethostbyname):
    ...
    @patch("intelstream.utils.url_validation.socket.gethostbyname")
    def test_blocks_cloud_metadata_endpoint(self, mock_gethostbyname):
        mock_gethostbyname.return_value = "169.254.169.254"
        is_safe, _ = is_safe_url("http://metadata.google.internal/")
        assert is_safe is False

    @patch("intelstream.utils.url_validation.socket.gethostbyname")
    def test_allows_public_url(self, mock_gethostbyname):
        mock_gethostbyname.return_value = "151.101.1.140"
    ...
    @patch("intelstream.utils.url_validation.socket.gethostbyname")
    def test_allows_when_dns_fails(self, mock_gethostbyname):
        import socket
